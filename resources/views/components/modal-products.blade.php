<!-- pineapple -->
<div id="pineapple" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.pineapple') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="pineapple-img" src="{{ asset('images/fruits/pineapple.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.pineapple_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- passion fruit -->
<div id="passion_fruit" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.passion_fruit') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="passion_fruit-img" src="{{ asset('images/fruits/passion_fruit.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.passion_fruit_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- strawberry -->
<div id="strawberry" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.strawberry') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="strawberry-img" src="{{ asset('images/fruits/strawberry.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.strawberry_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- mango -->
<div id="mango" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.mango') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="mango-img" src="{{ asset('images/fruits/mango.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.mango_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- tamarillo -->
<div id="tamarillo" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.tamarillo') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="tamarillo-img" src="{{ asset('images/fruits/tamarillo.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.tamarillo_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- guava -->
<div id="guava" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.guava') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="guava-img" src="{{ asset('images/fruits/guava.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.guava_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- lulo -->
<div id="lulo" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.lulo') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="lulo-img" src="{{ asset('images/fruits/lulo.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.lulo_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- tangerine -->
<div id="tangerine" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.tangerine') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="tangerine-img" src="{{ asset('images/fruits/tangerine.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.tangerine_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- blackberry -->
<div id="blackberry" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.blackberry') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="blackberry-img" src="{{ asset('images/fruits/blackberry.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.blackberry_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- soursop -->
<div id="soursop" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.soursop') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="soursop-img" src="{{ asset('images/fruits/soursop.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.soursop_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- curuba -->
<div id="curuba" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.curuba') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="curuba-img" src="{{ asset('images/fruits/curuba.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.curuba_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>

<!-- feijoa -->
<div id="feijoa" class="modal">
    <div class="modal-content">
        <span class="modal-content__close">&times;</span>
        <h2 class="badge badge--pink modal-title">{{ __('products.feijoa') }}</h2>
        <section class="fruit-description card">
            <div class="fruit-modal-image">
                <img id="feijoa-img" src="{{ asset('images/fruits/feijoa.png') }}" alt="">
            </div>
            <div class="fruit-description__text">
                <p>
                {{ __('products.feijoa_facts') }}
                </p>
            </div>
        </section>
        <section class="fruit-info">
            <div class="nutrition-facts">
                <h3 class="nutrition-facts__title badge badge--pink">{{ __('products.nutrition_facts') }}</h3>
                    <table class="nutrition-facts__table">
                        <tr>
                            <td>ENERGY</td>
                            <td>201kj/48 kcal</td>
                        </tr>
                        <tr>
                            <td>FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>SATURATED FAT</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>CARBOHYDRATE</td>
                            <td>13 g</td>
                        </tr>
                        <tr>
                            <td>SUGAR</td>
                            <td>0 g</td>
                        </tr>
                        <tr>
                            <td>FIBRE</td>
                            <td>1.1 g</td>
                        </tr>
                        <tr>
                            <td>PROTEIN</td>
                            <td>0.8 g</td>
                        </tr>
                        <tr>
                            <td>SALT</td>
                            <td>7 mg</td>
                        </tr>
                    </table>
            </div>
            <div class="fruit-preparation">
                <h3 class="fruit-preparation__title badge badge--pink">{{ __('products.preparation_tips') }}</h3>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">1</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
                <div class="fruit-preparation__tip">
                    <div class="fruit-preparation__bullet circle">2</div>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. 
                        Donec ornare leo ut mi consectetur pellentesque
                    </p>
                </div>
            </div>
        </section>
    </div>
</div>